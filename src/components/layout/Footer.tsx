import Link from "next/link";

export default function Footer() {
  return (
    <footer className="bg-[#050505] text-white">
      <div className="max-w-[1440px] mx-auto px-6 md:px-10 pt-16 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[220px_1fr_1fr_1fr_240px] gap-10 mb-12">
          {/* Brand */}
          <div>
            <p className="font-black text-xl tracking-[0.1em] text-white mb-3" style={{ fontFamily: "var(--font-display)" }}>NF MOTORS</p>
            <p className="text-white/40 text-sm leading-relaxed mb-6" style={{ fontFamily: "var(--font-body)" }}>
              Drive Your Dreams with<br />NF Motors Elite Luxury!
            </p>
            <div className="flex gap-3">
              {[
                { label: "Instagram", icon: <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg> },
                { label: "X", icon: <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg> },
                { label: "TikTok", icon: <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.89a8.18 8.18 0 004.78 1.52V7c0-.01-1.01-.31-1.01-.31z"/></svg> },
                { label: "LinkedIn", icon: <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg> },
              ].map(({ label, icon }) => (
                <a
                  key={label}
                  href="#"
                  aria-label={label}
                  className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-white/40 hover:text-white hover:border-white/30 transition-all duration-300"
                >
                  {icon}
                </a>
              ))}
            </div>
          </div>

          {/* Nav col 1 */}
          <div>
            <ul className="space-y-3">
              {[
                { label: "Domů", href: "/" },
                { label: "O nás", href: "/o-nas" },
                { label: "Nabídka vozů", href: "/nabidka" },
                { label: "Služby", href: "/sluzby" },
                { label: "Kontakt", href: "/kontakt" },
              ].map(({ label, href }) => (
                <li key={label}>
                  <Link href={href} className="text-white/50 text-sm hover:text-white transition-colors duration-200" style={{ fontFamily: "var(--font-body)" }}>
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Nav col 2 */}
          <div>
            <ul className="space-y-3">
              {[
                { label: "Financování", href: "/financovani" },
                { label: "Pojištění", href: "/sluzby" },
                { label: "Dovoz a registrace", href: "/sluzby" },
                { label: "Záruka", href: "/sluzby" },
                { label: "Kariéra", href: "/kontakt" },
              ].map(({ label, href }) => (
                <li key={label}>
                  <Link href={href} className="text-white/50 text-sm hover:text-white transition-colors duration-200" style={{ fontFamily: "var(--font-body)" }}>
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <ul className="space-y-3">
              {[
                { label: "info@nfmotors.com", href: "mailto:info@nfmotors.com" },
                { label: "+1 (123) 456-7890", href: "tel:+11234567890" },
                { label: "Miami, Florida", href: "/kontakt" },
                { label: "Tampa, Florida", href: "/kontakt" },
              ].map(({ label, href }) => (
                <li key={label}>
                  <a href={href} className="text-white/50 text-sm hover:text-white transition-colors duration-200 underline underline-offset-2" style={{ fontFamily: "var(--font-body)" }}>
                    {label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* CTA */}
          <div className="flex flex-col justify-between">
            <p className="text-white/60 text-sm leading-relaxed" style={{ fontFamily: "var(--font-body)" }}>
              Najděte top luxusní vozy rychle!
            </p>
            <Link
              href="/nabidka"
              className="mt-4 inline-flex items-center gap-2 px-6 py-3 border border-white/20 text-white text-sm font-semibold rounded-full transition-all duration-300 hover:bg-white hover:text-[#0B0B0B] w-fit"
              style={{ fontFamily: "var(--font-body)" }}
            >
              EXPLORE MODELS
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
              </svg>
            </Link>
          </div>
        </div>

        {/* Divider + Bottom */}
        <div className="border-t border-white/5 pt-6 flex flex-col md:flex-row justify-between items-center gap-2">
          <p className="text-white/20 text-xs" style={{ fontFamily: "var(--font-body)" }}>
            © 2025 NF Motors. Všechna práva vyhrazena.
          </p>
        </div>
      </div>
    </footer>
  );
}
